...
const TIP_CONTENT = {
    ...
    '设置一个 grunt（或者 rollup）任务来在本地编辑您的代码并将其提交到 Screeps。',
    ...
    '除非您每 tick 都用光了您的 CPU，不然没有用掉的部分会被存起来以备后续使用。',

    'Your CPU Limit depends on your Global Control Level.':
    '您的 CPU 上限取决于您的全局控制等级（GCL）。',
    ...
    '您可以 <code>transfer</code> 和 <code>heal</code> 另一个玩家的 creep，以及 <code>transfer</code>、<code>build</code> 和 <code>repair</code> 其他玩家的建筑。',
    ...
    '每个 creep 都会在 1500 tick 后死亡，然而您可以通过对其调用 <code>Spawn.renewCreep</code> 方法来延长它的寿命。',
