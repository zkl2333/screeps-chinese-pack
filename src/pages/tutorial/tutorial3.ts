...
const TUTORIAL_CONTENT = {
    ...
    '控制器升级解锁了新的建筑：墙、城墙以及扩展。我们将在下一节讨论墙和城墙，现在让我们先来了解一下扩展。',
    ...
    '<b>扩展</b> 被用来孵化更大型的 creep。每种身体类型只有一个部件的 creep 工作并不高效。多为其添加几个 <code>WORK</code> 部件可以让它们成比例的提高效率。',
    ...
    '但是，这样的 creep 会更加的昂贵，并且单独一个母巢只能容纳最多 300 点能量。想要孵化成本超过 300 点能量的 creep，您需要扩展。',
    ...
    '二级控制器将允许您建造 <strong>5 个扩展</strong>。每次控制器升级都会解锁更多的扩展。',
    ...
    '您可以在您房间中的任何位置放置扩展，并且母巢可以无视彼此之间的距离直接使用扩展中的能量。为了方便起见，我们已经放置好了对应的建筑工地。',
    ...
    '想要维护扩展，您需要教会您的采集单位把能量运输到扩展而不仅仅是母巢。为此，您需要使用 <code>Game.structures</code> 对象或者在对应的房间执行 <code>Room.find(FIND_STRUCTURES)</code> 方法进行搜索。无论使用哪种方式，您都需要用判断条件 <code>structure.structureType == STRUCTURE_EXTENSION</code>（或者 <code>structure\n instanceof StructureExtension</code>）对结果列表进行筛选，还有别忘了检查它们存有多少能量（就像之前检查 creep 一样）。',
    ...
    '想要了解房间里总共有多少能量可以用于孵化，您可以使用 <code>Room.energyAvailable</code> 属性。让我们把这个属性输出到控制台中以便在扩展填充期间对其进行追踪。',
    ...
    '现在我们的母巢和扩展中总共有 550 点能量。这已经足够建造一个身体部件为 <code>[WORK,WORK,WORK,WORK,CARRY,MOVE,MOVE]</code> 的 creep 了。这个 creep 的效率是普通工作单位的 4 倍。但是这也让它变得更重，所以我们给它添加了额外的 <code>MOVE</code> 部件。但是，两个 <code>MOVE</code> 也没办法让它跑得像小 creep 那样快，除非我们给他添加 4 个 <code>MOVE</code> 或者修一条路。',
    ...
    '您可以在右边的这个面板中看到，这个超强的 creep 每 tick 能采集 8 点能量。两三个这样的 creep 就可以在一个能量源恢复能量之前将其完全采干，并由此为您的殖民地带来最大化的能量收益。',
    ...
    '因此，通过升级您的控制器，建造新的扩展和更强大的 creep，您殖民地的效率将会被极大的提升。并且，您可以用大型的 creep 代替一群小型的 creep 来减少用于操控它们的 CPU，请记住它，这在多人游戏里非常重要。',
