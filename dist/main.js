...
// @supportURL   https://github.com/zkl2333/screeps-chinese-pack/issues
...
(function () {
    ...
    const textWrites = new WeakMap();
    const updateTextContent = function (text, content) {
        if (text.data === content)
            return;
        textWrites.set(text, content);
        text.data = content;
    };
    const isOwnTextMutation = function (mutation) {
    ...
    const protectObservers = new WeakMap();
    const reportTranslationError = function (target, error) {
    ...
    const updateText = function (el, content) {
        const newContent = content[TRANSLATE_TO];
        if (typeof newContent === 'string')
            updateTextContent(el, newContent);
        else if (typeof newContent === 'function') {
            const newText = newContent(el.data);
        ...
        return allQueryContents.filter(content => {
            ...
            targetElements.forEach((element, index) => {
                if (!isHTMLElement(element) || isExceptElement(element))
                    return;
                if (content.protect && element.isStandNode)
                    return;
                try {
    ...
    function listener (callbacks) {
        ...
        const onHashChange = function () {
            if (updateHash(callbacks))
                safelyChangeElements(callbacks.onElementChange, [document.body]);
        };
        // 启动监听
    ...
    const updateHash = function ({ onHashChange }) {
    ...
    const safelyChangeElements = function (onElementChange, nodes) {
        try {
    ...
    const content = {
        ...
        content: [
            ...
            },
            {
                ...
                'zh-CN': (text) => text.replace(/(\d+) room on (shard\d+)/, '$1 个房间，位于 $2'),
                'reuse': true
            },
    ...
    const content$1 = {
        ...
        content: [
            ...
            {
                'en-US': /^Today at /,
                'zh-CN': (text) => text.replace('Today at ', '今天 '),
                'reuse': true
            },
            {
                'en-US': /^Yesterday at /,
                'zh-CN': (text) => text.replace('Yesterday at ', '昨天 '),
                'reuse': true
            },
            {
                'en-US': /^(\d+) minute(s?) ago$/,
                'zh-CN': (text) => text.replace(/(\d+) minute(s?) ago/, '$1 分钟前'),
                'reuse': true
            },
            {
                'en-US': /^(\d+) hour(s?) ago$/,
                'zh-CN': (text) => text.replace(/(\d+) hour(s?) ago/, '$1 小时前'),
                'reuse': true
            },
            // 服务器维护
    ...
    const TIP_CONTENT = {
        ...
        'Set up a grunt task to write scripts on your local machine and commit them to Screeps.': '设置一个 grunt（或者 rollup）任务来在本地编辑您的代码并将其提交到 Screeps。',
        'Each game action has a constant cost of 0.2 CPU.': '每个会影响游戏世界的动作都有 0.2 CPU 的固定成本。',
        'Towers can aim at any object in a room even through walls and obstacles.': '塔可以透过墙和障碍物瞄准同房间中的任何对象。',
        'Power banks appear only in neutral rooms that divide living sectors on the map.': '超能宝库只会出现在地图上分隔各个活跃分区的中立过道房间中。',
        'Modular architecture of a script will allow easy testing of individual functions in the simulator.': '脚本的模块化架构使得您可以在模拟器中轻松测试单个函数。',
        'Test various game scenarios in the simulator in order to be prepared for surprises.': '在模拟器中测试各种游戏场景，以应对随时可能发生的意外。',
        'Sources in neutral rooms have reduced capacity. Reserve or claim the room to restore it to full capacity.': '中立房间中能量源的容量较低。预订（reserve）或占领（claim）房间可以使其恢复到最大容量。',
        'To save your CPU, use less creeps of a larger size.': '生成数量更少、身体部件更多的 creep 来节省您的 CPU。',
        'Spawn extensions capacity increases on room levels 7 and 8.': 'RCL7 和 RCL8 将提升扩展的容量。',
        'Use towers to set up automatic defense of your room.': '使用塔来建立您房间的自动防御。',
        ...
        'A tower’s effectiveness depends on the distance to the target.': '塔的工作效率取决于该塔到目标的距离。',
        'You can create any objects in the simulator to test your script.': '您可以在模拟器中创建任何对象来测试脚本。',
        'Unless you use up your CPU limit each tick, it is stored for future use.': '除非您每 tick 都用光了您的 CPU，不然没有用掉的部分会被存起来以备后续使用。',
        'Your CPU Limit depends on your Global Control Level.': '您的 CPU 上限取决于您的全局控制等级（GCL）。',
        'You can use more CPU than your CPU limit allows in short bursts.': '您的 CPU 使用量可以在短时间内超过您的 CPU 上限。（“短时间”取决于 CPU 桶中的余额）',
        'Energy in a storage can not be used to spawn creeps. Transfer it to a spawn or extensions instead.': '仓库里储存的能量不能直接用来孵化 creep，要先将能量转移到一个母巢或扩展中。',
        'The more body parts of one type a creep has, the greater its productivity.': '一个 creep 拥有的同类型身体部件越多，该类型对应的工作效率就越高。',
        'More spawns in a room allows building more creeps at a time.': '一个房间中存在的母巢越多，能同时孵化的 creep 也就越多。',
        'The more spawn extensions in a room, the more energy you can spend on building one creep.': '一个房间中的母巢和扩展越多，可以用来孵化单个 creep 的能量也就越多。',
        'You can address from your script only those rooms that contain your creeps or structures.': '只有房间中存在您的 creep 或者建筑时，您的代码才可以访问到它。',
        'Ramparts can be built not just on empty squares but on existing structures too.': '城墙不仅可以在空旷的地块上建造，还可以建造在已有的建筑上。',
        'Ramparts and walls initially have 1 hit point. Repair them after construction.': '城墙和墙最初仅有 1 点生命值（hit），记得在建造好后及时进行维修（repair）。',
        ...
        'While not destroyed, a rampart protects a creep or building on its square from any type of attack.': '只要一个城墙没有被摧毁，它就可以保护同地块上的 creep 或者建筑免受任何形式的攻击。',
        'The game is fully recorded, so you can see replay of any room for the past several days.': '游戏已经被完整录制，所以您可以随时回放过去几天发生的事情。',
        'The more small objects in the Memory, the more CPU spent on its parsing.': 'Memory 中的小对象越多，解析它们所消耗的 CPU 就越多。',
        ...
        'Creeps can miss each other if they walk towards each other simultaneously or follow step by step.': '两个相邻的 creep 如果同时相向移动，或者前后紧跟着移动，可能会彼此错过。',
        'If you want to play from scratch, you can always Respawn in a new room.': '如果您想从头开始玩，您可以随时在一个新房间里重生。',
        'You can output HTML content to the console, like links to rooms.': '您可以将 HTML 内容输出到控制台，例如一个跳转到指定房间的超链接。',
        'You can have as many rooms under your control as your Global Control Level.': '您可以控制的房间数与全局控制等级（GCL）一样多。',
        'You can apply <code>transfer</code> and <code>heal</code> to another player’s creep, and <code>transfer,</code> <code>build</code> and <code>repair</code> to others’ structures.': '您可以 <code>transfer</code> 和 <code>heal</code> 另一个玩家的 creep，以及 <code>transfer</code>、<code>build</code> 和 <code>repair</code> 其他玩家的建筑。',
        ...
        'Spawn extensions do not have to be placed near spawns, their range is the whole room.': '扩展不用放在母巢的边上，它们的有效范围是整个房间。',
        'You can speed up downgrading of hostile room controller by using <code>Creep.attackController</code> on it.': '您可以通过使用 <code>Creep.attackController</code> 方法来加速敌对房间控制器的降级。',
        'To output an object content into the console, use <code>JSON.stringify</code>.': '要将对象内容输出到控制台，请使用 <code>JSON.stringify</code>。',
        'Build roads to save on <code>MOVE</code> body parts of your creeps.': '建造道路可以让您的 creep 使用更少的 <code>MOVE</code> 部件。',
        'Always try to control as many rooms as your GCL allows. It will allow your colony to develop at the maximum speed.': '始终尝试控制 GCL 所允许的房间数量，这将可以使您的殖民地以最大的速率发展。',
        ...
        'Walls, roads, and containers don’t belong to any player, so they should be searched with the help of <code>FIND_STRUCTURES</code>, not <code>FIND_MY_STRUCTURES</code>.': '墙、道路和容器不属于任何玩家，所以搜索它们需要使用 <code>FIND_STRUCTURES</code> 而不是 <code>FIND_MY_STRUCTURES</code> 常量。',
        ...
        'Use <code>Room.energyAvailable</code> and <code>Room.energyCapacityAvailable</code> to determine how much energy all the spawns and extensions in the room contain.': '使用 <code>Room.energyAvailable</code> 和 <code>Room.energyCapacityAvailable</code> 来确定房间中所有母巢和扩展包含多少能量及能量上限是多少。',
        'Observers allow to get the <code>Room</code> object for the rooms that have no objects of yours.': '观察器可以让您获取那些没有己方 creep 或建筑的房间所对应的 <code>Room</code> 对象。',
        'To control a room continuously, you need to upgrade your controller from time to time.': '想要持续控制一个房间，您需要经常升级您的房间控制器。',
        'The <code>Game.notify</code> function automatically groups identical messages using the specified interval.': '<code>Game.notify</code> 会按照指定的时间间隔自动合并相同的消息。',
        'Dead body parts have weight and generate fatigue as well.': '失效的身体部件仍有重量，也会产生疲劳。',
        ...
        'You can build and repair roads and containers in any rooms, even neutral ones.': '您可以在任何房间，哪怕是中立房间中建造和维修道路和容器。',
        'To prevent other players from seizing a neutral room you want, use <code>Creep.reserveController</code>.': '使用 <code>Creep.reserveController</code> 可以防止其他玩家占领您想要的中立房间。',
        'Creeps cannot move faster than 1 square per tick.': 'creep 的速度上限是 1 格/tick。',
        'Send emails to yourself with the function <code>Game.notify</code> to be aware of everything happening in the game.': '您可以通过使用 <code>Game.notify</code> 方法向自己发送电子邮件，以了解游戏中发生的一切。',
        ...
        'Every creep dies after 1500 ticks, however you can prolong its life using the <code>Spawn.renewCreep</code> method.': '每个 creep 都会在 1500 tick 后死亡，然而您可以通过对其调用 <code>Spawn.renewCreep</code> 方法来延长它的寿命。',
        ...
        'A creep with an <code>ATTACK</code> part automatically strikes back at every attacker by <code>ATTACK</code>.': '拥有 <code>ATTACK</code> 部件的 creep 会使用 <code>ATTACK</code> 自动反击每个攻击者。',
        'A spawn automatically replenishes itself with power until the energy in the room reaches 300 units.': '当房间中用于孵化的能量小于 300 时，母巢将会自动开始恢复能量，直到其能量等于 300 点。',
        'Leaderboards reset to zero each month, while your game process continues.': '排行榜每个月都会重置，您的游戏进度并不会受到影响。',
        'Use links to save on creep building and CPU.': '使用 Link 可以减少需要生产的 creep 数量并节省 CPU。',
        'Use storage to not lose surplus of mined resources.': '使用仓库来存储开采出来的过量资源。'
    };
    const tips = [
        { 'en-US': 'Do you want to turn off tips of the day?', 'zh-CN': '您想关闭每日提示吗？', 'reuse': true },
        { 'en-US': 'Tip of the day:', 'zh-CN': '每日提示：', 'reuse': true },
        { 'en-US': 'Don\'t show tips', 'zh-CN': '不再显示提示', 'reuse': true },
        { 'en-US': 'Next tip', 'zh-CN': '下一个提示', 'reuse': true },
        {
        ...
        },
        {
            ...
            'zh-CN': '在新手区（Novice Area）中您最多可以控制 3 个房间。',
            'reuse': true
        }
    ];
    /**
    ...
    const TUTORIAL_CONTENT = {
        ...
        'If you experience any performance issues, please note that Screeps is best played in Chrome browser.': '如果您遇到任何性能问题，请注意，使用 Chrome 浏览器可以获得最佳的 Screeps 游戏体验。',
        ...
        'The object in the center of the screen is your first spawn, your colony center.': '屏幕中心的这个小东西是您的第一个母巢，它是您的殖民地核心。',
        'You play by writing code in the panel in the bottom of the screen.': '您将通过在屏幕底部的面板中编写代码来进行游戏。',
        'You can enter your code in this field. It will run once.': '您可以通过在这个输入框中输入代码来执行它，代码只会运行一次。',
        ...
        'Your spawn creates new units called "creeps" by its method <code>spawnCreep</code>.\nUsage of this method is described in the <a href="http://docs.screeps.com" app-nw-external-link="" target="_blank">documentation</a>. Each creep has a name and certain body parts that give it\nvarious skills.': '您的母巢可以通过 <code>spawnCreep</code> 方法创建名为 “creep” 的新单位。可以在 <a href="https://screeps-cn.gitee.io/index.html" app-nw-external-link="" target="_blank">本文档</a> 中找到该方法的介绍。每个 creep 都有一个名字（name）和一定量的身体部件（body part），不同的身体部件会带来不同的能力。',
        'You can address your spawn by its name the following way: <code>Game.spawns[\'Spawn1\']</code>.': '您可以使用母巢的名字来获取到它，就像这样：<code>Game.spawns[\'Spawn1\']</code>。',
        ...
        'It is time to put the creep to work! This yellow square is an energy source — a valuable game resource.\nIt can be harvested by creeps with one or more <code>WORK</code> body parts and transported to the spawn by creeps with <code>CARRY</code> parts.': '现在是时候让这个 creep 去工作了！这个黄色小方块是一个能量源（Source） —— 一种宝贵的游戏资源。它可以被带有一个或多个 <code>WORK</code> 身体部件的 creep 采集，并由带有 <code>CARRY</code> 部件的 creep 运送到母巢。',
        ...
        'To commit a script to the game so it can run, use this button or <strong>Ctrl+Enter</strong>.': '使用此按钮或按 <strong>Ctrl+Enter</strong> 向游戏提交脚本，使其开始运行。',
        ...
        'You give orders to a creep by its name this way: <code>Game.creeps[\'Harvester1\']</code>.\nUse the <code>FIND_SOURCES</code> constant as an argument to the <code>Room.find</code> method.': '您可以通过 creep 的名字来获取到它并对其下达命令，就像这样：<code>Game.creeps[\'Harvester1\']</code>。把 <code>FIND_SOURCES</code> 常量作为参数传递给 <code>Room.find</code> 方法，就可以找到房间中的能量源。',
        ...
        'To make the creep transfer energy back to the spawn, you need to use the method\n<code>Creep.transfer</code>.\nHowever, remember that it should be done when the creep is next to the spawn, so the creep needs to walk back.': '想要让 creep 把能量运送回母巢，您需要使用 <code>Creep.transfer</code> 方法。但是请记住，这个方法只有在 creep 和母巢相邻的时候才能正确执行，所以需要让 creep 先走回来。',
        'If you modify the code by adding the check <code>.store.getFreeCapacity()&nbsp;&gt;&nbsp;0</code> to the creep,\nit will be able to go back and forth on its own, giving energy to the spawn and returning to the source.': '当您把 <code>.store.getFreeCapacity()&nbsp;&gt;&nbsp;0</code> 作为检查条件添加到代码里时，creep 应该就可以自己一步步的把能量搬运回母巢然后走回能量源。',
        'Great! This creep will now work as a harvester until it dies. Remember that almost any creep has a life cycle of 1500\ngame ticks, then it "ages" and dies (this behavior is disabled in the Tutorial).': '太好了！现在这个 creep 将会一直作为采集者（harvester）工作直到死亡。请记住，几乎所有的 creep 都有 1500 游戏 tick 的生命周期，在此之后它就会“老去”然后死亡（这个设定在本教程中并不生效）。',
        ...
        'By adding new roles and modules to your creeps this way, you can control and manage the work of many creeps.\nIn the next Tutorial section, we’ll develop a new creep role.': '通过这种方法向您的 creep 添加新的角色和模块，由此控制和管理众多 creep 的工作。在下一节里，我们将开发一个新的 creep 角色。'
    };
    /**
     * 行动目标文本
     */
    const OBJECTIVE_CONTENT = {
        ...
        'Extend the creep program so that it can transfer harvested energy to the spawn and return back to work.': '拓展您的 creep 程序，使其可以将采集到的能量运送（transfer）回母巢中并重新开始工作。',
    ...
    const TUTORIAL_CONTENT$1 = {
        ...
        'All your stored memory is accessible via the global <code>Memory</code> object. You can use it any way you like.': '您储存的所有内存信息都可以通过全局对象 <code>Memory</code> 访问。您可以按任意方式使用它。',
    ...
    const TUTORIAL_CONTENT$2 = {
        'The Controller upgrade gives access to some new structures: walls, ramparts, and extensions.\nWe’ll discuss walls and ramparts in the next Tutorial section, for now let’s talk about extensions.': '控制器升级解锁了新的建筑：墙、城墙以及扩展。我们将在下一节讨论墙和城墙，现在让我们先来了解一下扩展。',
        '<b>Extensions</b> are required to build larger creeps. A creep with only one body part of one type works poorly.\nGiving it several <code>WORKs</code> will make him work proportionally faster.': '<b>扩展</b> 被用来孵化更大型的 creep。每种身体类型只有一个部件的 creep 工作并不高效。多为其添加几个 <code>WORK</code> 部件可以让它们成比例的提高效率。',
        'However, such a creep will be costly and a lone spawn can only contain 300 energy units.\nTo build creeps costing over 300 energy units you need spawn extensions.': '但是，这样的 creep 会更加的昂贵，并且单独一个母巢只能容纳最多 300 点能量。想要孵化成本超过 300 点能量的 creep，您需要扩展。',
        'The second Controller level has <strong>5 extensions</strong> available for you to build.\nThis number increases with each new level.': '二级控制器将允许您建造 <strong>5 个扩展</strong>。每次控制器升级都会解锁更多的扩展。',
        'You can place extensions at any spot in your room, and a spawn can use them regardless of the distance.\nIn this Tutorial we have already placed corresponding construction sites for your convenience.': '您可以在您房间中的任何位置放置扩展，并且母巢可以无视彼此之间的距离直接使用扩展中的能量。为了方便起见，我们已经放置好了对应的建筑工地。',
    ...
    const TUTORIAL_CONTENT$4 = {
        'The world of Screeps is not the safest place. Other players may have claims on your territory.\nBesides, your room may be raided by neutral NPC creeps occasionally. So you ought to think about your\ncolony defense in order to develop it successfully.': 'Screeps 的世界并不安全。其他玩家可能想要占领您的领土。此外，您的房间也会偶尔遭到中立 NPC creep 的袭击。所以，您需要好好考虑殖民地的防御，这样才能更安全地发展自己。',
        'This hostile creep has come from the left entry and attacked your colony. It’s good that we have walls to\nrestrain it temporarily. But they will fall sooner or later, so we need to deal with the problem.': '敌方 creep 从房间左边的入口入侵并袭击了您的殖民地。由于我们有墙可以暂时抵挡它的进攻，所以目前问题不大。但是墙迟早会被打穿，所以我们需要尽快解决这个问题。',
        ...
        'As you can see, the enemy creep stopped attacking the wall – its harmful methods are blocked.\nWe recommend that you activate safe mode when your defenses fail.': '如您所见，敌方 creep 已经不再进攻墙了——它的有害操作被阻止了。我们建议您在房间的防御失效时再激活安全模式。',
        'Now let’s cleanse the room from unwanted guests.': '现在，让我们把这些不速之客清理掉。',
        'Towers are the easiest way to actively defend a room. They use energy and can be targeted at any creep in a room\nto attack or heal it. The effect depends on the distance between the tower and the target.': '塔是防御房间最简单直接的手段。它们可以消耗能量来治疗或攻击房间中的任何 creep。治疗/攻击效果取决于塔和目标之间的直线距离。',
        'To start with, let’s lay a foundation for our new tower. You can set any place you wish inside the walls\nand place the construction site there with the help of the button “Construct” on the upper panel.': '首先，让我们给新的塔打好地基。您可以在墙之内的任何位置放置塔的建筑工地，通过顶部面板中的“建造”按钮找到它。',
        ...
        'A tower uses energy, so let’s set the harvester role to bring energy to the tower along with other structures.\nTo do this, you need to add the constant <code>STRUCTURE_TOWER</code> to the filter of structures your\nharvester is aimed at.': '塔需要能量，所以让我们改造一下 harvester 角色，让其可以把能量带到塔和其他建筑中。想要实现这个功能，您需要将 <code>STRUCTURE_TOWER</code> 常量添加到用于筛选您采集单位目标的 filter 中。',
        'Excellent, your tower is ready to use!': '棒极了，您的塔已经准备就绪了！',
        'Like a creep, a tower has several similar methods: <code>attack</code>, <code>heal</code>, and\n<code>repair</code>. Each action spends 10 energy units. We need to use <code>attack</code> on the closest\nenemy creep upon its discovery. Remember that distance is vital: the effect can be several times stronger\nwith the same energy cost!': '就像 creep 一样，塔也有几个类似的方法：<code>attack</code> - 攻击，<code>heal</code> - 治疗，以及 <code>repair</code> - 维修。每个操作都会消耗 10 点能量。一旦发现了敌人，我们就需要使用 <code>attack</code> 方法攻击距离最近的敌方 creep。请记住，距离非常重要：在相同的能量消耗下，操作带来的效果可能会有好几倍的差距。',
        'To get the tower object directly you can use its ID from the right panel and the method <code>Game.getObjectById</code>.': '想要获取塔的对象，您可以使用它的 ID（右侧面板中）以及 <code>Game.getObjectById</code> 方法。',
        'The enemy creep is eliminated and our colony can breathe easy. However, the invader has damaged some walls during the brief\nattack. You’d better set up auto-repair.': '敌方 creep 被消灭，我们终于可以松口气了。但是，在刚才短暂的袭击中，入侵者还是对一些墙造成了伤害。您最好设置一下自动维修机制。',
        'Damaged structures can be repaired by both creeps and towers. Let’s try to use a tower for that.\nWe’ll need the method <code>repair</code>. You will also need the method <code>Room.find</code> and a filter to locate the damaged walls.': 'creep 和塔都可以修复受损的建筑，这次让我们用塔来试一下。使用 <code>repair</code> 方法可以完成这个任务。除此之外，您还需要使用 <code>Room.find</code> 方法和一个 filter 去筛选那些受损的墙。',
        'Note that since walls don’t belong to any player, finding them requires the constant <code>FIND_STRUCTURES</code>\nrather than <code>FIND_MY_STRUCTURES</code>.': '请注意，由于墙不属于任何玩家，所以我们需要使用 <code>FIND_STRUCTURES</code> 常量进行搜索而不是 <code>FIND_MY_STRUCTURES</code>。',
    ...
    const OBJECTIVE_CONTENT$4 = {
        ...
        'Place the construction site for the tower (manually or using the code below).': '放置塔的建筑工地（手动或使用下面的代码）。',
        'Add <code>STRUCTURE_TOWER</code> to the module <code>role.harvester</code> and wait for the energy to appear in the tower.': '在 <code>role.harvester</code> 模块中添加 <code>STRUCTURE_TOWER</code>，然后等待能量运送到塔中。',
        'Destroy the enemy creep with the help of the tower.': '使用塔消灭敌方 creep。',
        'Repair all the damaged walls.': '修复所有受损的墙。'
    };
    /**
     * 帮助文档更换
     */
    ...
    const content$b = {
        ...
        content: [
            ...
            { 'en-US': 'You need 1 free Power Level in your account to create a new Power Creep.', 'zh-CN': '您需要一个账户内空余的超能等级来创建新的超能 creep。' },
            ...
            // OPERATE_LAB
            { 'en-US': 'Increase reaction amount by', 'zh-CN': '提高实验室的反应产物数量 ', 'reuse': true },
            // OPERATE_OBSERVER
            ...
            { 'en-US': 'for this creep?', 'zh-CN': ' 为该 creep 提升等级吗？', 'reuse': true },
            {
                ...
                'zh-CN': '开启 24 小时实验期后，您可以测试超能 creep 的配置而不会损失等级。实验期间：',
                'reuse': true
            },
            {
                'en-US': 'Power Creeps are deleted immediately without delay.',
                'zh-CN': '超能 creep 可以被立刻删除，而不会启动删除倒计时。',
                'reuse': true
            },
            {
                'en-US': 'You don\'t lose Global Power Levels when you delete a Power Creep.',
                'zh-CN': '删除超能 creep 时不会损失全局超能等级。',
                'reuse': true
            },
            {
                'en-US': /You have \d+ periods left. Would you like to activate it?/,
                'zh-CN': (text) => text.replace('You have', '您还剩 ').replace('periods left. Would you like to activate it?', '个实验期。是否要激活一个？'),
                'reuse': true
            },
            {
...
`;
    ...
    const content$f = {
        ...
        content: [
            ...
            { 'en-US': 'Shard', 'zh-CN': '镜面', 'reuse': true },
    ...
    const content$m = {
        ...
        content: [
            ...
            { 'en-US': 'Old Password', 'zh-CN': '旧密码' },
            { 'en-US': 'New Password', 'zh-CN': '新密码' },
            ...
            { 'en-US': 'Once again please.', 'zh-CN': '请再次输入。', 'reuse': true },
            { 'en-US': 'Save', 'zh-CN': '保存' },
            { 'en-US': 'Cancel', 'zh-CN': '取消' }
        ]
    };
    ...
    const content$p = {
        ...
        content: [
            ...
            { 'en-US': 'World shards are isolated from each other and run your code separately.', 'zh-CN': '各个世界镜面彼此独立，并分别运行您的代码。' },
            { 'en-US': 'Your creeps can travel between them using special portals.', 'zh-CN': '您的 creep 可通过特殊传送门在不同镜面之间穿行。' },
            { 'en-US': 'Learn more', 'zh-CN': '了解更多' },
            ...
            { 'en-US': 'Cancel', 'zh-CN': '取消' },
            // 分配确认提示
            { 'en-US': 'You\'re going to re-assign your CPU to the following shards:', 'zh-CN': '您将把 CPU 重新分配给以下镜面：', 'reuse': true },
            ...
            { 'en-US': 'Do you want to proceed?', 'zh-CN': '确定要继续吗？', 'reuse': true },
            { 'en-US': 'Ok', 'zh-CN': '确定', 'reuse': true },
            { 'en-US': 'Cancel', 'zh-CN': '取消', 'reuse': true }
        ]
    };
    ...
    const GRAPH_SELECT_LIST$1 = {
        'None': '无',
        'Owner control level': '所有者的房间控制等级',
        'Claimable': '是否可被占领',
        'Minerals': '矿物类型',
        'Power enabled': '超能启用状态',
        ...
        content: [
            {
                ...
                'zh-CN': '切换到支持装饰与地图视觉效果（即将推出）的新版世界地图'
            },
            {
            ...
            { 'en-US': 'Display:', 'zh-CN': '显示：', 'reuse': true },
            ...
            { 'en-US': 'Room', 'zh-CN': '房间', 'reuse': true },
            { 'en-US': 'Novice area', 'zh-CN': '新手区', 'reuse': true },
            { 'en-US': 'days left', 'zh-CN': '天剩余', 'reuse': true },
            ...
            { 'en-US': 'Energy harvested:', 'zh-CN': '能量采集量：', 'reuse': true },
    ...
    const content$s = {
        ...
        content: [
            ...
            },
            {
                ...
                'zh-CN': (el) => {
                    el.innerHTML = el.innerHTML.replace('<em>Real programming</em>, not just pseudocode. You can use libs from real projects.', '<em>用真实的代码编程</em>，而不是伪代码。您可以使用实际项目中的库。');
            ...
            },
            {
                ...
                'zh-CN': (el) => {
                    ...
                    el.innerHTML = el.innerHTML.replace('<em>40-server cluster</em> (160 CPU cores) processing player scripts using Node.js.', '由 <em>40 台服务器组成的集群</em>（160 个 CPU 核心）使用 Node.js 处理玩家脚本。');
            ...
            { 'en-US': 'What people say', 'zh-CN': '人们的评价' },
            {
                ...
                'zh-CN': (el) => {
                    ...
                    el.innerHTML = el.innerHTML.replace(/(\d+)%&nbsp;of&nbsp;([\d,]+)&nbsp;reviews are positive/, '$2\u00A0条评价中有\u00A0$1%\u00A0为好评');
                }
            },
            {
