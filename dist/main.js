...
(function () {
    ...
    const TIP_CONTENT = {
        ...
        'Towers can aim at any object in a room even through walls and obstacles.': 'tower 可以透过墙和障碍物瞄准同房间中的任何对象。',
        'Power banks appear only in neutral rooms that divide living sectors on the map.': '超能量仓只会出现在地图上分隔各个活跃分区的中立过道房间中。',
        ...
        'Sources in neutral rooms have reduced capacity. Reserve or claim the room to restore it to full capacity.': '中立房间中资源点的容量较低。预订（reserve）或占领（claim）房间可以使其恢复到最大容量。',
        ...
        'Use towers to set up automatic defense of your room.': '使用防御塔来建立您房间的自动防御。',
        ...
        'A tower’s effectiveness depends on the distance to the target.': '防御塔的工作效率取决于该防御塔到目标的距离。',
        ...
        'Ramparts and walls initially have 1 hit point. Repair them after construction.': 'Rampart 和墙最初仅有 1 点生命值（hit），记得在建造好后及时进行维修（repair）。',
    ...
    const TUTORIAL_CONTENT$2 = {
        'The Controller upgrade gives access to some new structures: walls, ramparts, and extensions.\nWe’ll discuss walls and ramparts in the next Tutorial section, for now let’s talk about extensions.': '控制器升级解锁了新的建筑：墙、Rampart 以及 Extension。我们将在下一节讨论墙和 Rampart，现在让我们先来了解一下 Extension。',
    ...
    const TUTORIAL_CONTENT$4 = {
        ...
        'To start with, let’s lay a foundation for our new tower. You can set any place you wish inside the walls\nand place the construction site there with the help of the button “Construct” on the upper panel.': '首先，让我们给新的防御塔打好地基。您可以在墙之内的任何位置放置防御塔的建筑工地，通过顶部面板中的“建造”按钮找到它。',
        ...
        'Damaged structures can be repaired by both creeps and towers. Let’s try to use a tower for that.\nWe’ll need the method <code>repair</code>. You will also need the method <code>Room.find</code> and a filter to locate the damaged walls.': 'creep 和防御塔都可以修复受损的建筑，这次让我们用防御塔来试一下。使用 <code>repair</code> 方法可以完成这个任务。除此之外，您还需要使用 <code>Room.find</code> 方法和一个 filter 去筛选那些受损的墙。',
...
`;
    ...
    const content$p = {
        ...
        content: [
            ...
            { 'en-US': 'World shards are isolated from each other and run your code separately.', 'zh-CN': '各个世界分片彼此独立，并分别运行您的代码。' },
            { 'en-US': 'Your creeps can travel between them using special portals.', 'zh-CN': '您的 creep 可通过特殊传送门在不同分片之间穿行。' },
            { 'en-US': 'Learn more', 'zh-CN': '了解更多' },
            ...
            { 'en-US': 'Cancel', 'zh-CN': '取消' },
            // 分配确认提示
            { 'en-US': 'You\'re going to re-assign your CPU to the following shards:', 'zh-CN': '您将把 CPU 重新分配给以下分片：', 'reuse': true },
